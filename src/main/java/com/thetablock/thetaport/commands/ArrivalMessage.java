...
@Cmd(name = "tparrival", usage = "/tparrival", aliases = {}, enabled = true, label = "", permission = "")
public class ArrivalMessage extends CommandHandler implements Injectors {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        if (sender.hasPermission(permission)) {
            if (args.length > 1) {
                ...
                switch (response) {
                    case SUCCESS:
                        sender.sendMessage("Arrival has been set for " + args[0]);
                        break;
                    case INVALID_PORT:
